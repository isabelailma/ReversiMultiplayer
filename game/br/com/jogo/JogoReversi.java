...
public class JogoReversi implements Serializable {
	...
	public JogoReversi(Jogador player1, Jogador player2) {
		...
		for (int i = 0; i < tabuleiro.length; i++) {
			for (int j = 0; j < tabuleiro[i].length; j++) {
				...
				tabuleiro[i][j]
						.addMouseListener(new BoardCellMouseEvents(this));
			}
		}

		tabuleiro[3][3].setarDono(player1, false);
		tabuleiro[4][4].setarDono(player1, false);
		tabuleiro[3][4].setarDono(player2, false);
		tabuleiro[4][3].setarDono(player2, false);
	...
	void cellClicked(ActionEvent event) {
		...
		if (atual.isHumano() && atual.isValido()) {
